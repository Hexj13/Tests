...
class Template_mixin(object):
	...
	DEFAULT_TITLE = 'Flexbby Test Report'
	DEFAULT_DESCRIPTION = ''
	...
	<table id='result_table' class="table">
	    ...
	    <tfoot>
	        <tr id='total_row'>
	            ...
	            <td class="text text-warning">%(fail)s</td>
	            <td class="text text-danger">%(error)s</td>
	            <td>&nbsp;</td>
	        </tr>
	    </tfoot>
	</table>
...
class HTMLTestRunner(Template_mixin):
	...
	def _generate_report(self, result):
		...
		for cid, (cls, cls_results) in enumerate(sortedResult):
			...
			row = self.REPORT_CLASS_TMPL % dict(
				style=ne > 0 and 'text text-danger bg-danger' or nf > 0 and 'text text-warning bg-warning' or 'text text-success bg-success',
				desc=desc,
				count=np + ns + nf + ne,
				Pass=np,
				fail=nf,
	...
	def _generate_report_test(self, rows, cid, tid, n, t, o, e):
		...
		row = tmpl % dict(
			...
			style=n == 2 and 'text text-danger' or (n == 1 and 'text text-warning' or 'text text-success'),
			desc=desc,
			script=script,
