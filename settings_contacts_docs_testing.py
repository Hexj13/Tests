...
class ContactsDocsTesting(unittest.TestCase):
	print("----------------------------------------", flush=True)
	print(TextColors.HEADER + "Test 'ContactsDocsTesting' START" + TextColors.ENDC, flush=True)
	print("----------------------------------------", flush=True)
	print("----------------------------------------", flush=True)
	...
	def test_contacts_docs(self):
		self.toolkit.login(login_text, password_text)
		print(TextColors.WARNING + "test_contacts_docs START" + TextColors.ENDC, flush=True)
		print("", flush=True)
		# Спим
		...
		# Переходим в Перс. документы
		self.toolkit.clickByXPATH(menu_button_xpath % 'Перс. документы')
		...
		time.sleep(SleepSeconds.THREE)
		print("", flush=True)
		print(TextColors.WARNING + "test_contacts_docs END" + TextColors.ENDC, flush=True)
		print("----------------------------------------", flush=True)
	...
	def tearDown(self):
		...
		print("----------------------------------------", flush=True)
		print(TextColors.HEADER + "Test 'ContactsDocsTesting' FINISH" + TextColors.ENDC, flush=True, end="")
